...
function renderAssetCards(q=''){
  ...
  el.innerHTML=list.map(a=>{const s=assetScore(a), cls=assetStatusClass(a), w=warrantyState(a), related=tickets.filter(t=>t.asset===a.id);return `<button class="cmdb-asset-card ${cls}" onclick="openAsset360('${esc(a.id)}')"><div class="asset-top"><span class="asset-big-icon">${assetIcon(a)}</span><div><strong>${esc(a.nome)}</strong><small>${esc(a.id)} • ${esc(a.tipo)}</small></div></div><div class="asset-health"><span>Saúde</span><b>${s}%</b><i><em style="width:${s}%"></em></i></div><div class="asset-mini"><span>Usuário <b>${esc(a.usuario)}</b></span><span>Local <b>${esc(a.local)}</b></span><span>Garantia <b class="${w.cls}">${w.label}</b></span><span>Chamados <b>${related.length}</b></span></div></button>`}).join('')||'<div class="empty-state">Nenhum ativo encontrado.</div>';
}
