...
const KEY='tosi-support-pro-v14';
...
const navItems=[['dashboard','⌂','Dashboard'],['tickets','▣','Chamados'],['newTicket','＋','Novo Chamado'],['clientPortal','◉','Portal do Usuário'],['kanban','▦','Kanban'],['workflow','⟲','Workflow'],['automations','⚡','Automações'],['serviceCatalog','◈','Catálogo de Serviços'],['assets','▥','Ativos TI / CMDB'],['knowledge','▤','Base de Conhecimento'],['reports','▧','Relatórios'],['settings','⚙','Configurações']];
...
function renderClientPortal(){
  ...
  portalCatalog.innerHTML=catalog.map(s=>`<button class="portal-service" onclick="showPage('newTicket');ticketCategory.value='${esc(s.name.includes('senha')?'Acesso / Senha':s.name.includes('Impressoras')?'Impressoras':s.name.includes('Rede')?'Rede / Internet':s.name.includes('E-mail')?'E-mail / Microsoft 365':'Suporte Técnico')}'"><span>${s.icon}</span><div><strong>${esc(s.name)}</strong><small>SLA ${esc(s.sla)} • ${esc(s.owner)}</small></div></button>`).join('')||'<div class="empty-state">Nenhum serviço encontrado.</div>';
  ...
  portalKb.innerHTML=kb.map(k=>`<div class="portal-kb-item"><strong>${esc(k[0])}</strong><p>${esc(k[1])}</p><button onclick="alert('Artigo aberto em modo demonstração.')">Ler artigo</button></div>`).join('')||'<div class="empty-state">Nenhum artigo encontrado.</div>';
}
